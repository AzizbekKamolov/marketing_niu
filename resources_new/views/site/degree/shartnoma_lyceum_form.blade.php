        ...
        <div class="row">
            <div class="signup-content" style="margin: auto;">
                <div class="signup-form">
                    <h2 class="form-title"> Shartnoma olish </h2><br>
                   @if(session('info_error'))
                    <p class="error">
                        {{ session('info_error') }}
                    </p>
                   @endif
                    <form action="{{ route('shartnoma.lyceum_info') }}" method="post" name="super-form" class="register-form" id="register-form">
                        {{ @csrf_field() }}
                        ...
                        <div class="form-group" style="display: flex; flex-wrap: wrap;">
                             ...
                             <span class="error" style="">
                                ...
                                <input type="text" class="number-inputmask" name="id_code" id="id_code" autocomplete="off" value="{{ old('id_code') }}" placeholder=" O'quvchi ID raqami"/>
                            
                        </div>
                        <div class="form-group">
                            <span class="error" style="">
                                @if ($errors->has('parent_pass_seria'))
                                        {{ $errors->first('parent_pass_seria') }}
                                    @endif
                            </span>
                            <label for="parent_pass_seria">
                                <i class="zmdi zmdi-account-box-mail"></i></label>
                            <input type="text" name="parent_pass_seria" id="passport_seria" value="{{ old('parent_pass_seria') }}" placeholder="Vasiy pasport seriyasi"/>
                        </div>
                        <div class="form-group">
                             <span class="error" style="">
                                @if ($errors->has('parent_pass_number'))
                                        {{ $errors->first('parent_pass_number') }}
                                    @endif
                            </span>
                            <label for="parent_pass_number"><i class="zmdi zmdi-confirmation-number"></i></label>
                            <input type="text" name="parent_pass_number" id="passport_number" value="{{ old('parent_pass_number') }}" placeholder="Vasiy pasport raqami"/>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" onfocus="(this.type='date')" name="birthday" value="{{ old('birthday') }}" id="birthday" placeholder="O'quvchi tug'ilgan sanasi"/>
